...
class cafe_management():
        ...
        def Total_Bill(self):
                self.tea_price = 5000
                self.coffee_price = 8000
                self.sandwitch_price = 15000
                self.cake_price = 22000
                self.burger_price = 22000
                self.pizza_price = 35000
                self.fries_price = 8000
                self.pepsi_price = 8000
                ...
                if self.service_cost != "":
                        ...
                        self.service_cost.insert(END,1000.0)
                else:
                        self.service_cost.insert(END,1000.0)
                if self.sub_cost != "":
                ...
                if self.paid_tax != "":
                        ...
                        self.paid_tax.insert(END,float(self.sub_cost.get())*10/100)
                else:
                        self.paid_tax.insert(END,float(self.sub_cost.get())*10/100)
                ...
                if self.bill_details.get(1.0,"end") != "":
                        ...
                        #self.bill_details.insert(1.0,f"PASTE HERE")
                        self.bill_details.insert(1.0,f" Billno-{random.randint(100,1000)}\t{date}  \n Items(q) ======= \tAmount  ======= \n {'Tea ('+str(self.tea_item.get()) + ')' + ' ============> ' + str(self.tea_cost) + ' '  if self.tea_item.get() != 0 else''}\n{' coffee ('+str(self.coffee_item.get()) + ')' + ' ============> ' + str(self.coffee_cost) + '  '  if self.coffee_item.get() !=0 else ''}\n{ ' cake ('+str(self.cake_item.get()) + ')' + ' ============> ' + str(self.cake_cost) + '  '  if self.cake_item.get() != 0 else''}\n{' sandwitch ('+str(self.sandwitch_item.get()) + ')' + ' ============> ' + str(self.sandwitch_cost) + ' '  if self.sandwitch_item.get() !=0 else''}\n{' burger('+str(self.burger_item.get()) + ')' + ' ============> ' + str(self.burger_cost) + ' '  if self.burger_item.get() !=0 else''}\n{' pizza('+str(self.pizza_item.get()) + ')' + ' ============> ' + str(self.pizza_cost) + ' '  if self.pizza_item.get() !=0 else''}\n{' fries('+str(self.fries_item.get()) + ')' + ' ============> ' + str(self.fries_cost) + ' '  if self.fries_item.get() != 0 else''}\n{' pepsi('+str(self.pepsi_item.get()) + ')' + ' ============> ' + str(self.pepsi_cost) + ' '  if self.pepsi_item.get() !=0 else''}\n\n item cost ------- {self.items_cost.get()} \n service cost ------- {self.service_cost.get()} \n=====================\n Sub Cost {self.sub_cost.get()} \n=====================\n tax paid ------- {self.paid_tax.get()}\n ================= \n total ------- {self.total_bill.get()}\n =====================")
                        #self.bill_details.insert(1.0,"tes tes tes")
        ...
        def __init__(self):
                ...
                self.root.title("UNS MADIUN CAFE")
                self.root.maxsize(500,300)
                self.root.minsize(500,300)
                self.root['bg'] = "white"

                self.heading = Label(self.root,text="UNS MADIUN CAFE",font=('verdana',20,'bold'),fg="#248aa2",bg="white")
                ...
                self.cafe_icon = ImageTk.PhotoImage(Image.open('Logo_UNS.png'))
                self.logo = Label(self.root,image=self.cafe_icon,bg="white")
                self.logo.place(x=240,y=70)
                ...
                self.frame2 = LabelFrame(self.root,text="Cafe Items Bills",width=180,height=160,font=('verdana',10,'bold'),
                ...
                self.multiply = Button(self.frame3,text="*",padx=7,relief=RAISED,borderwidth=2,font=('verdana',10,'bold'),
                                       bg='white',fg="black",command=self.mul)
                ...
                self.Save_Bills_btn.place(x=413,y=245)
        
                self.icon = ImageTk.PhotoImage(Image.open('false.png'))
                self.Clear_btn = Button(self.root,image=self.icon,relief=RAISED,borderwidth=2,font=('verdana',10,'bold'),bg='#248aa2',
                                        fg="white",padx=5,command=self.Clear)
                self.Clear_btn.place(x=465,y=245)
